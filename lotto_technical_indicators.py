...
WEIGHTS = {
    "z":     0.35,   # Z-Score          : 52회차 전체 사용 → 가장 안정적 (35%)
    "bb":    0.20,   # 볼린저밴드 %B     : 26회차 횡단면, 중기 트렌드 보완 (20%)
    "rsi":   0.20,   # RSI               : 14회차 모멘텀, 단기 과열/냉각 직관적 반영 (20%)
    "ma":    0.15,   # MA 크로스오버     : 4/26 윈도우 단기 변동성 반영 (15%)
    "aroon": 0.10,   # Aroon 오실레이터  : 26회차 갭 재귀성, 보조 지표 (10%)
}
...
def main(
    ...
    weight_info = [
        ("Z-Score",        f"{WEIGHTS['z']*100:.0f}%",
         "52회차 전체 사용 → 1년 기준 가장 안정적 편차 지표 (35%)"),
        ("볼린저밴드 %B",  f"{WEIGHTS['bb']*100:.0f}%",
         "26회차 횡단면 → Z-Score 단기 보완, 반년 빈도분포 포착 (20%)"),
        ("RSI",            f"{WEIGHTS['rsi']*100:.0f}%",
         "14회차 모멘텀 → 과열/냉각 직관적 포착, 단기 지표 (20%)"),
        ("MA 크로스오버",  f"{WEIGHTS['ma']*100:.0f}%",
         "4vs26 단기/중기 대비로 신호 강도 향상 (15%)"),
        ("Aroon",          f"{WEIGHTS['aroon']*100:.0f}%",
         "26회차 갭 재귀성 → 반년 내 최장공백 번호 포착 보조 지표 (10%)"),
    ]
...
if __name__ == "__main__":
    main(
        ...
        ma_short_w    = 4,     # 단기: 최근 4회차 (1개월) — 단기 트렌드 반영
        ma_long_w     = 26,    # 장기: 26회차 (반년) — 중장기 기준선
        rsi_window    = 14,    # 단기 모멘텀 (14회차 표준)
        bb_window     = 26,    # 6개월 횡단면 (52의 50%)
        aroon_window  = 26,    # 반년 갭 패턴 (52의 50%)
        include_bonus = True,
        seed          = 42,
    )
